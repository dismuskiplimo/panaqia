<div class="remodal" data-remodal-id="add-event-contact" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc">
              ...
              <select name="type" id="contact_type" class = "browser-default" required="">
                @if(count($contact_types))
                  @foreach($contact_types as $contact_type)
                    <option value="{{ $contact_type->name }}">{{ $contact_type->name }}</option>
                  @endforeach
                @else
